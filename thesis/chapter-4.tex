...
\end{theorem}
\begin{proof}
Разобьем вектор параметров модели учителя на зануляемые $\boldsymbol{\nu}_1$, удаляемые $\boldsymbol{\nu}_2$ и сохраняемые $\boldsymbol{\upsilon}$ параметры и обозначим $\bar{\boldsymbol{\nu}}_1=[\boldsymbol{\upsilon}^{\mathsf{T}},\boldsymbol{\nu}_2^{\mathsf{T}}]^{\mathsf{T}}$. Из свойств многомерного нормального распределения следует, что условное распределение $p(\bar{\boldsymbol{\nu}}_1|\mathfrak{D},\boldsymbol{\nu}_1=\mathbf{0})$ также является нормальным с параметрами
\[
    \boldsymbol{\mu}
    =
    \mathbf{m}_{\bar{\boldsymbol{\nu}}_1}
    ...
    \boldsymbol{\Sigma}_{\boldsymbol{\nu}_1,\bar{\boldsymbol{\nu}}_1},
\]
где $\mathbf{m}_{\bar{\boldsymbol{\nu}}_1}$ и $\mathbf{m}_{\boldsymbol{\nu}_1}$~--- подвекторы $\mathbf{m}$, соответствующие параметрам $\bar{\boldsymbol{\nu}}_1$ и $\boldsymbol{\nu}_1$, а $\boldsymbol{\Sigma}_{\bar{\boldsymbol{\nu}}_1,\boldsymbol{\nu}_1}$~--- соответствующая подматрица $\boldsymbol{\Sigma}$.
Распределение сохраняемых параметров $\boldsymbol{\upsilon}$ получается маргинализацией условного распределения по удаляемым параметрам $\boldsymbol{\nu}_2$. Маргинал нормального распределения также нормален, поэтому
\[
    p(\boldsymbol{\upsilon}|\mathfrak{D})=\mathcal{N}(\boldsymbol{\mu}_{\boldsymbol{\upsilon}},\ \boldsymbol{\Xi}_{\boldsymbol{\upsilon},\boldsymbol{\upsilon}}),
\]
где $\boldsymbol{\mu}_{\boldsymbol{\upsilon}}$~--- подвектор $\boldsymbol{\mu}$, соответствующий параметрам $\boldsymbol{\upsilon}$, а $\boldsymbol{\Xi}_{\boldsymbol{\upsilon},\boldsymbol{\upsilon}}$~--- соответствующая подматрица $\boldsymbol{\Xi}$.
\end{proof}
...
\end{theorem}
\begin{proof}
Рассмотрим сеть с $T+1$ слоем и ее фрагмент, содержащий $t$-й слой. Так как размеры соседних слоев совпадают, матрица $\mathbf{U}_t$ квадратная. Если матрицу параметров $t$-го слоя приравнять к единичной, то сеть с $T+1$ слоем эквивалентна сети с $T$ слоями:
\[
    f = \boldsymbol{\sigma}\circ\mathbf{U}_{T+1}\boldsymbol{\sigma}\circ\ldots\circ\mathbf{U}_t\boldsymbol{\sigma}\circ\ldots\circ\mathbf{U}_1
    =
    \boldsymbol{\sigma}\circ\mathbf{U}_{T+1}\boldsymbol{\sigma}\circ\ldots\circ\mathbf{I}\boldsymbol{\sigma}\circ\ldots\circ\mathbf{U}_1
    =
    \boldsymbol{\sigma}\circ\mathbf{U}_{T+1}\boldsymbol{\sigma}\circ\ldots\circ\boldsymbol{\sigma}\circ\ldots\circ\mathbf{U}_1,
\]
где последний переход использует идемпотентность $\boldsymbol{\sigma}\circ\boldsymbol{\sigma}=\boldsymbol{\sigma}$:
\[
    f = \boldsymbol{\sigma}\circ\mathbf{U}_{T+1}\boldsymbol{\sigma}\circ\ldots\circ\mathbf{U}_1.
\]
Тем самым удаление $t$-го слоя эквивалентно приравниванию матрицы параметров этого слоя к единичной матрице, что задается вектором $\mathbf{i}$ из условия теоремы. Распределение параметров после приравнивания к единичной матрице вычисляется при помощи условного распределения. В силу свойств нормального распределения оно также является нормальным с параметрами
\[
    \boldsymbol{\mu}
    =
    \mathbf{m}_{\boldsymbol{\upsilon}}
    ...
    \boldsymbol{\Sigma}_{\bar{\boldsymbol{\upsilon}},\boldsymbol{\upsilon}},
\]
где $\mathbf{m}_{\boldsymbol{\upsilon}}$~--- подвектор $\mathbf{m}$, соответствующий сохраняемым параметрам $\boldsymbol{\upsilon}$, а $\boldsymbol{\Sigma}_{\boldsymbol{\upsilon},\bar{\boldsymbol{\upsilon}}}$~--- соответствующая подматрица $\boldsymbol{\Sigma}$. Следовательно,
\[
    p(\boldsymbol{\upsilon}|\mathfrak{D})
    =
    \mathcal{N}(\boldsymbol{\mu}_{\boldsymbol{\upsilon}},\ \boldsymbol{\Xi}_{\boldsymbol{\upsilon},\boldsymbol{\upsilon}}).
\]
\end{proof}
...
\end{enumerate}
Гипотеза проверяется эмпирически в разделе~\ref{sec:anti-distillation-experiments} и считается подтвержденной, если выполнены условия~1--3.
\end{hypothesis}
...
\begin{figure}[h!t]\center
    ...
    \caption{Зависимость среднеквадратического отклонения прогноза от процента удаленных параметров для различных методов прореживания на синтетической выборке с мультиколлинеарными параметрами. Метод Белсли здесь дает уменьшение ошибки при удалении шумовых параметров.}
    \label{Data1All}
\end{figure}
...
На рис.~\ref{Data1All} показана зависимость среднеквадратического отклонения прогноза от точного ответа от процента удаленных параметров для различных методов. По графику удаление параметров методом Белсли дает меньшую ошибку, чем сравниваемые методы прореживания, в том числе при удалении шумовых параметров.
...
В байесовской дистилляции информация учителя передается ученику через априорное распределение параметров. Приведены вариационная постановка и утверждения о сохранении нормального вида распределения при удалении нейрона и слоя в полносвязной модели. На синтетических данных и Fashion-MNIST использование апостериора учителя как основы априора ученика повышало логарифм правдоподобия вдоль траектории обучения.

Многодоменная дистилляция передает знания между моделями на доменах, связанных инъективным отображением~$\varphi$, через функцию потерь с метками учителя и композицией~$(f\circ\varphi)$. На ImageNet варианты с учителем и доменной адаптацией улучшали точность и потери относительно обучения без учителя; обучение в одном домене оставалось сильнее варианта с адаптацией. На OPUS-100 для перевода снижалась перекрестная энтропия и росла метрика BLEU при использовании учителя.

Антидистилляция передает знания от более простой модели к более сложной. Составная функция потерь включает перекрестную энтропию, близость параметров, устойчивость к шуму во входах и регуляризацию следа матрицы Гессе~(оценка через гессиан-векторное умножение). На Fashion-MNIST инициализация антидистилляцией давала валидационную точность~$0{,}86\pm 0{,}05$ (Xavier~$0{,}68\pm 0{,}08$, transfer learning~$0{,}74\pm 0{,}09$) и более высокую устойчивость к FGSM и к нормальному шуму в параметрах среди сравниваемых инициализаций.
