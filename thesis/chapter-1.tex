...
Для проверки полученных теоретических оценок мы провели детальное эмпирическое исследование.
...
Для достижения этой цели мы обучили полносвязную нейронную сеть на полном наборе данных и получили соответствующие параметры $\hat{\boldsymbol{\theta}}$ как точку вблизи минимума.
Впоследствии мы исследовали зависимость между средней разностью потерь и доступным размером выборки.

Мы использовали библиотеку \texttt{pytorch}~\cite{pytorch2019} в качестве Python-фреймворка для обучения нейронных сетей.
...
Размер $h$ был фиксированным для всех скрытых слоев $L$.
Сеть обучалась в течение многочисленных эпох с использованием оптимизатора \texttt{Adam}~\cite{adam2015} с постоянной скоростью обучения $10^{-3}$.
Использовались различные наборы данных для классификации изображений, доступные в библиотеке \texttt{torchvision}.
...
В этом эксперименте мы использовали значения пикселей изображений в качестве входных данных.
...
Мы выбрали Vision Transformer (ViT)~\cite{wu2020visual} от Google.
Мы аналогичным образом выбрали случайным образом~$10\,000$ объектов из набора данных MNIST и варьировали размер скрытого слоя и количество слоев.
...
Наш эксперимент подтверждает сходимость, доказанную в Теореме~\ref{chapter:complex:theorem-kiselev-loss}.
...
Мы выбрали четыре набора данных из библиотеки \texttt{torchivison}: MNIST~\cite{deng2012mnist}, FashionMNIST~\cite{xiao2017fashionmnistnovelimagedataset}, CIFAR10 и CIFAR100~\cite{krizhevsky2009learning}.
Единственной предобработкой данных являлась нормализация для приведения значений к диапазону~$[-1; 1]$.

На Рис.~\ref{chapter:complex:experiment:fig:additional-exp} графики слева были получены при фиксированном количестве слоев $L=5$ в сети.
Мы изменяли размер скрытого слоя на всех уровнях от $4$ до $64$.
...
Данная последовательность была повторена $100$ раз для усреднения. Для полученных результатов мы применили экспоненциальное скользящее среднее с коэффициентом сглаживания $0.99$.
...
Для достижения этой цели мы обучили сверточные сети и получили параметры $\hat{\boldsymbol{\theta}}$ вблизи оптимума.
Мы использовали сверточную архитектуру с функцией активации ReLU после каждого слоя.
Чтобы проследить влияние конкретного параметра на сходимость, мы фиксировали ключевые параметры нейронной сети, варьировали интересующий гиперпараметр и обучали соответствующий набор моделей.
Затем мы исследовали зависимость между средним абсолютным различием между значениями средней функции потерь и доступным размером выборки.
Далее, для каждой модели, чтобы получить более надежные результаты, мы усредняли разность потерь по перемешанным выборкам.
Дополнительно, для улучшения визуализации, мы использовали экспоненциальное сглаживание с коэффициентом $0.995$.
Для данного исследования мы использовали числовое представление пикселей изображений в качестве входных данных.
...
Для более глубокого изучения зависимости между функцией потерь и ее гессианом мы проводим эксперимент, соответствующий Теореме~\ref{chapter:complex:theorem-transformer-loss}.
Здесь мы используем другую конфигурацию модели на наборе данных CIFAR-100~\cite{krizhevsky2009learning}.
...
\begin{enumerate}
    \item Обучаем модель до сходимости и сохраняем вектор параметры~$\mathbf{w*}.$
    \item Начинаем с пустого набора данных, добавлять данные батч за батчем (англ. batch) и вычисляем среднее значение потерь по просмотренным батчам;
    \item Вычисляем абсолютную разность в соответствии с выражением:
        \[
