...
Для определения достаточности используется функция правдоподобия. 
...
Таким образом, формализуем критерии, позволяющие определить достаточный объем выборки.
...
В приведенных выше определениях вместо функции правдоподобия~$L(\mathfrak{D}_m, \hat{\mathbf{w}}_{k})$ рассматривается ее логарифм~$l(\mathfrak{D}_m, \hat{\mathbf{w}}_{k})$.
...
К сожалению нет какого-либо теоретического обоснования для этого, но на практике такая эвристика оказывается рабочей.
...
Рассмотрим две похожие подвыборки~$\mathfrak{D}_k = (\mathbf{X}_k,\mathbf{y}_k)$ и~$\mathfrak{D}_{k+1} = (\mathbf{X}_{k+1}, \mathbf{y}_{k+1})$ размеров~$k$ и~$k+1$ соответственно, то есть выборка~$\mathfrak{D}_{k+1}~$ получена путем добавления одного элемента к выборке~$\mathfrak{D}_k$.
...
Теорема~\ref{chapter:samplesize:theorem-kiselev-posterior-similiarity} указывает, что расстояние между двумя гауссовскими распределениями стремится к нулю по мере сходимости их векторов математических ожиданий и ковариационных матриц.
...
В первой части проверяются сходимости, полученные в ходе теоретического анализа.
Далее оцениваются размеры выборок для различных наборов данных, используя разные подходы.
Наконец, изучаются зависимость достаточного размера выборки от объема доступных данных.

Для начала исследуем, наблюдаются ли на практике полученные теоретические сходимости.
А именно, сначала рассматрим поведение минимального собственного значения матрицы~$\mathbf{X}_k^\top \mathbf{X}_k$ при увеличении размера выборки.
Затем исследуем сходимость предложенных функций~$KL(k)$ и~$S(k)$.
...
Для генерации синтетического набора данных регрессии, происходит генерация исходных признаков, параметров модели и шумовых остатков из стандартного нормального распределения.
Априорное распределение параметров также задано как стандартное нормальное, как для синтетической регрессии, так и для набора данных Liver Disorders, который содержит 345 объектов и 5 признаков.
Входные признаки предобрабатываются, используя стандартный метод масштабирования данных~(англ. Standard Scaler).
...
Для каждого размера выборки~$k$ вычисляется минимальное собственное значение матрицы~$\mathbf{X}_k^\top \mathbf{X}_k$.
Также вычисляется значения~$KL(k)$ и~$S(k)$. Этот процесс повторялся~$B=100$ раз.
На Рис.~\ref{chapter:samplesize:experiment:fig:eigvals} показано асимптотическое поведение минимального собственного значения матрицы~$\mathbf{X}_k^\top \mathbf{X}_k$.
Видно, что когда размер выборки стремится к бесконечности, минимальное собственное значение также стремится к бесконечности.
...
В то же время, на Рис.~\ref{chapter:samplesize:experiment:fig:liver-disorders} представлены аналогичные графики для набора данных Liver Disorders.
...
На Рис.~\ref{chapter:samplesize:experiment:fig:sufficient-vs-threshold} показана, зависимость между уровнем достоверности и размеров выборки.
...
Далее проводится наиболее комплексный анализ различных методов определения размера выборки.
Проанализируем, как достаточный размер выборки зависит от объема доступного набора данных.
В частности, увеличивая объем доступной выборки и вычисляя достаточный размер на основе различных методов.
Таким образом, получаем Рис.~\ref{chapter:samplesize:experiment:fig:dependence_on_available_sample_set}, который сравнивает вышеупомянутые методы с точки зрения их консервативности.

Видно, что S-достаточный размер выборки часто является минимальным.
