...
\begin{property}[Неравенство для нормы блочной матрицы]\label{prop:block_matrix_norm}
    Пусть задана матрица~$\mathbf{A} \in \mathbb{R}^{m \times n}$ представляет собой блочную матрицу, каждый блок которой является матрицей $\mathbf{B}_{i,j}$. Тогда выполняется следующее неравенство:
    \begin{equation}
        \| \mathbf{A} \|_2 \leq \sqrt{m n} \max\limits_{i,j}\| \mathbf{B}_{i,j}\|_2
    \end{equation}
    
    Заметим, что если матрица~$\mathbf{A}$ является блочно-диагональной, то имеет место строгое равенство~$\| \mathbf{A} \|_2 = \max\limits_{i}\| \mathbf{B}_{i,i}\|_2$.
\end{property}

\begin{property}[Поэлементное деление]\label{prop:elem_wise_division}
    Пусть задана матрица~$\mathbf{A} \in \mathbb{R}^{m\times n}$ и вектор~$\mathbf{b} \in \mathbb{R}^{m \times 1}$.
    Тогда для матрицы $\mathbf{C} \in \mathbb{R}^{m \times n}$, где $c_{i,j} = \frac{a_{i,j}}{b_i}$, выполняется
    \begin{equation}
...
\begin{property}[Производная матричного произведения]\label{prop:matrix_product_derivative}
    Пусть заданы матрицы~$\mathbf{X}, \mathbf{A}, \mathbf{B}$ соответствующих размерностей, тогда
    \begin{equation}
...
Данный результат непосредственно следует из \cite{magnus1988matrix}, где был получен аналогичный результат для покоординатной векторизации.
\end{property}
...
\begin{definition}[Матрица перестановки]\label{def:commutation_matrix}
    Матрица перестановки~$\mathbf{K}_{m,n} \in \mathbb{R}^{mn \times mn}$ это такая единственная матрица, что для любой матрицы~$\mathbf{A} \in \mathbb{R}^{m \times n}$ выполняется:
    \begin{equation}
        \mathbf{K}_{m,n} \mathrm{vec}(\mathbf{A}) = \mathrm{vec}(\mathbf{A}^\top)
    \end{equation}
    Используя свойство~\ref{prop:vec_relation}, получаем соотношение:
    \begin{equation}
...
Пусть задана матрица~$\mathbf{A}$ и вектор~$\mathbf{v}$. Тогда
\begin{itemize}
    \item $\mathrm{vec}_r(\mathbf{A})$ обозначает построчную векторизацию матрицы $\mathbf{A}$.
    \item $\mathbf{A}^{\circ \alpha}$ обозначает поэлементное возведение матрицы $\mathbf{A}$ в степень $\alpha$, т.е. $(\mathbf{A}^{\circ \alpha})_{ij} = (\mathbf{A}_{ij})^\alpha$.
    \item $\textit{diag}(\mathbf{v})$ создаёт диагональную матрицу с вектором $\mathbf{v}$ на главной диагонали.
\end{itemize}
\end{definition}

\begin{definition}\label{def:matrix_norms}
Для матрицы $\mathbf{A} \in \mathbb{R}^{m \times n}$:
\begin{align}
    \|\mathbf{A}\|_2 &= \sigma_1, \\
