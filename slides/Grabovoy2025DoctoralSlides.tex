...
\title[Ландшафтная мера сложности моделей и данных]{Ландшафтная мера сложности моделей и данных \\в параметрических моделях машинного обучения}
\author{А.В. Грабовой}
...
\begin{enumerate}
    \justifying
    \item[--] введение формальных определений мер сложности моделей и данных в рамках теории мер и установление критерия согласованности модели и выборки;
...
\begin{itemize}
    \justifying
    \item Введены формальные определения меры сложности выборки $\mu_D(D)$ и меры сложности модели $\mu_f(f)$ в рамках теории мер, установлен критерий согласованности $\mu_f(f) \le \mu_D(D)$.
    ...
    \item Предложены методы снижения сложности: прореживание на основе ковариации градиентов, многодоменная дистилляция и антидистилляция.
    \item Доказаны теоремы о состоятельности LoRA-адаптеров и снижении радемахеровской сложности в многозадачном обучении.
\end{itemize}
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\begin{itemize}
    \justifying
    \item \textbf{Отсутствие единого формального языка.} Введение мер сложности $\mu_D(D)$ и $\mu_f(f)$; критерий согласованности $\mu_f(f) \le \mu_D(D)$.
    ...
    \item \textbf{Отсутствие теоретически обоснованных методов снижения сложности.} Прореживание на основе ковариации градиентов; многодоменная дистилляция; антидистилляция.
    \item \textbf{Эмпирический характер законов масштабирования.} Теоремы о состоятельности LoRA; снижение радемахеровской сложности в многозадачном обучении в $1/\sqrt{T}$ раз.
\end{itemize}
...
\begin{tikzpicture}[node distance=1.2cm and 2.2cm, auto, scale=1.0, transform shape]
    ...
    \node[draw, rectangle, rounded corners, fill=gray!10, text width=2.2cm, text centered, minimum height=0.8cm, font=\footnotesize] (context1) at (0,3.5) {Классические:\\VC, PAC,\\радемахеровская сложность};
...
\begin{itemize}
    ...
    \item Эффективная емкость, оцениваемая через след матрицы Гессе.
...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Критерий согласованности и дообучение}
\justifying
...
Это минимальная сложность подвыборки, достаточная для выполнения критерия согласованности.
...
Ландшафтная мера характеризует отклонение матриц Гессе по объектам выборки:
\begin{itemize}
...
\begin{rustheorem}[\emph{Мешков-Киселев-Грабовой}]
    ...
    где~$C_{\mathbf{W}^{(i)}}$~--- одномерная свертка с ядром~$\mathbf{W}^{(i)}$. Пусть заданы следующие верхние оценки на параметры:~$C_l \leqslant C, k_i \leqslant k_{\mathrm{ker}}, d_i \leqslant d_1:=d, |\mathbf{W}^{(p)}_{i,j,k}|^2 \leqslant w^2.$
    Тогда норма матрицы Гессе имеет следующую верхнюю оценку: $\left\|\mathbf{H}\right\| \leqslant \sqrt{2}\left\|x\right\|^2d^2(L+1)(C^2w^2k_{\mathrm{ker}}d)^L.$
\end{rustheorem}
...
\begin{rustheorem}[\emph{Мешков-Киселев-Грабовой}]
    ...
    Пусть заданы следующие верхние оценки на параметры:~$C_l \leqslant C, k_i \leqslant k_{\mathrm{ker}}, m_i \leqslant m_1:=m, n_i \leqslant n_1:=n, |\mathbf{W}^{(p)}_{i,j,k}|^2 \leqslant w^2.$
    Тогда норма матрицы Гессе имеет следующую верхнюю оценку:~$\left\|\mathbf{H}_{O}\right\| \leqslant \sqrt{2}\left\|\mathbf{x}\right\|^2C^4k_{\mathrm{ker}}^4m^2n^2(L+1)(C^2k_{\mathrm{ker}}^2w^2mn)^{L}.$
\end{rustheorem}
...
\hline
1D свертка & $L(C^2w^2k_{\mathrm{ker}}d)^L$ \\
\hline
2D свертка & $L(C^2k_{\mathrm{ker}}^2w^2mn)^L$ \\
\hline
...
\begin{rusremark}
    ...
    В отличие от этого, данный анализ сохраняет размер выборки целевой задачи $n$ фиксированным и сравнивает STL и MTL на одном и том же $n$, поэтому улучшение проявляется как множитель $1/\sqrt{T}$ в радемахеровской сложности на задачу.
\end{rusremark}
...
\begin{itemize}
    ...
    \item Согласуется с теоретическим снижением радемахеровской сложности в многозадачном обучении.
\end{itemize}
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\begin{enumerate}
    \justifying
    \item Разработанный единый теоретический аппарат оценки сложности моделей глубокого обучения и сложности данных на основе теории мер и анализа ландшафта оптимизационной задачи, включающий формальные определения мер сложности и критерий согласованности модели и выборки.
    ...
    \item Предложенные методы снижения сложности моделей глубокого обучения на основе анализа ковариационной матрицы градиентов, дистилляции на многодоменных данных и антидистилляции обеспечивают эффективное сокращение числа параметров и передачу знаний между моделями различной сложности и доменами данных.
\end{enumerate}
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\tiny
\begin{enumerate}
    \item Grabovoy~A.\;V. Landscape Measure of Deep Learning Models // Pattern Recognition and Image Analysis: Advances in Mathematical Theory and Applications. 2026. Vol.~36, no.~2. pp.~708--716. Pleiades Publishing, Ltd (Road Town, United Kingdom).

    \item Киселев~Н.\;С., Мешков~В.\;С., Грабовой~А.\;В. Устойчивая сходимость поверхности функции потерь через усреднение по распределению // Труды Института системного программирования РАН (электронный журнал). 2026. Т.~38, №~3. С.~71--82.
...
которых 25 статей в научно-технических журналах, входящих в перечень ВАК, 30~--- в изданиях, входящих в международные наукометрические базы Scopus и Web of Science. В трудах российских и международных конференций опубликовано 39 работ. Также результаты работ автора использовались при регистрации 13 программ для ЭВМ.
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\begin{enumerate}
    \justifying
    \item Разработан единый формальный аппарат для оценки сложности моделей и данных на основе теории мер и анализа ландшафта оптимизационной задачи. Введены формальные определения меры сложности выборки и меры сложности модели, установлен критерий согласованности модели и выборки.
    ...
    \item Критерии достаточного объема выборки по близости апостериорных распределений сходятся в модели линейной регрессии.
    \item Разработаны методы многодоменной дистилляции и антидистилляции для передачи знаний между моделями различной сложности и между различными доменами данных. Для многозадачного обучения доказаны теоремы о статистической состоятельности низкоранговых адаптеров и снижении эмпирической сложности.
\end{enumerate}
\end{frame}
%----------------------------------------------------------------------------------------------------------
\end{document}